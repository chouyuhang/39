...
$httpClient = new \chouyuhang\LINEBot\HTTPClient\CurlHTTPClient(getenv('WI8f+ot/+7IJffBJATgfi1+rnNYCW+RGm1u2SRg2sdOLw2Y0+4gbdJsmh0zmUdtZNvx595o+hvI3XYeFQk66EVpl1mWwDDJOlKRecD6mc8gES9hnbAH+SOcrxw3QWmrmvQPI0WxrXMwB8EVOXPx4FwdB04t89/1O/w1cDnyilFU='));
...
foreach ($events as $event) {
  if ($event instanceof \chouyuhang\LINEBot\Event\PostbackEvent) {
    error_log('postback : ' . file_get_contents('php://input')); // date data in postback field
  }

  if ($event instanceof \chouyuhang\LINEBot\Event\MessageEvent) {
    if ($event instanceof \chouyuhang\LINEBot\Event\MessageEvent\TextMessage) {
      $json = array(
